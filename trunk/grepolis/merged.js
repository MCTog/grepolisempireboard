...
Ajax.get = function (controller, action, params, callbackSuccess, options, lock, show_ajax_loader) {
    Ajax.ajax(controller, action, params, callbackSuccess, options, "GET", lock, show_ajax_loader);
}
Ajax.post = function (controller, action, params, callbackSuccess, options, lock, show_ajax_loader) {
    Ajax.ajax(controller, action, params, callbackSuccess, options, "POST", lock, show_ajax_loader);
}
Ajax.ajax = function (controller, action, params, callbackSuccess, options, type, lock, show_ajax_loader) {
...
jQuery.blocker = function (options) {
    ...
    this.blocker.block = function () {
        elm.original_parent = settings.html.parent()
...
jQuery.fn.countdown = function (until, options) {
    ...
    var interval = window.setInterval(tick.bind(this), options.interval ? options.interval : 100);
...
var Layout = {
    ...
    updateBar: function (bar) {
        ...
        if (bar.god) {
            $('#god_mini').hide();
            $('#god_mini').attr('src', $('#god_mini').attr('src').replace(/favor_[a-z]*\.png/, 'favor_' + bar.god + '.png'));
            $('#god_mini').fadeIn('slow');
            Layout.god = bar.god;
        }
    ...
    initializeResourcesProductionCounter: function () {
        ...
        $(document).everyTime(100, function () {
            ...
            if (Layout.favor < Layout.max_favor) {
                Layout.favor_offset += Layout.favor_production / (60 * 60 * 10);
    ...
    startPlaytimeCountdown: function () {
        var time = $('#playtime_hint');
        time.countdown(Layout.playtime_countdown_finished_at, {});
        time.unbind();
        time.bind('finish', function () {
            var title = 'Spielzeit';
            var text = 'Es ist nun eine weitere Stunde vergangen!';
            Layout.showHint(title, text, 'Layout.reset_playtime_hint()');
        });
    },
    ...
    reset_playtime_hint: function () {
        Ajax.post('player_hint', 'reset_playtime_countdown', {}, function (data) {
            $('#player_hint').remove();
            Layout.playtime_countdown_finished_at = data.playtime_countdown_finished_at;
            Layout.startPlaytimeCountdown();
        });
    },
    ...
    initializeResourcesProductionCounterForOverview: function (towns, overview_type) {
        Layout.towns = towns;
        for (town in Layout.towns) {
            ...
            for (town in Layout.towns) {
                for (res_id in resources) {
                    ...
                    if (Layout.towns[town].resources[id] < Layout.towns[town].storage_volume) {
                        Layout.towns[town].resources[id + '_offset'] += Layout.towns[town].production[id] / (60 * 60 / (step / 1000));
                        if (Layout.towns[town].resources[id + '_offset'] > 1) {
                            ...
                            if (Layout.towns[town].resources[id] >= Layout.towns[town].storage_volume) {
                                ...
                                var resource_elem = $('#town_' + Layout.towns[town].id + (overview_type == 'buildings' ? '_res' : '') + ' .' + id);
                                resource_elem.addClass('town_storage_full');
                            }
                            if (overview_type != 'trade') {
                                var elem = $('#town_' + Layout.towns[town].id + (overview_type == 'buildings' ? '_res' : '') + ' .resource_' + id + '_icon');
                            } else {
                                var elem = $('#town_' + Layout.towns[town].id + '_' + id);
                            }
                            var icon = (overview_type == 'buildings' || overview_type == 'trade' || overview_type == 'hide') ? elem.html().match(/<(.|\n)*?>/g) : null;
                            elem.html((icon != null ? icon[0] + icon[1] : '') + Layout.towns[town].resources[id]);
                        }
                    }
                }
            }
...
var PopupFactory = {
    ...
    init: function () {
        ...
        ts[pfs[i].i + '_info'] = '<img src="http://static.grepolis.com/images/game/premium_features/' + pfs[i].i + '.jpg" alt="' + pfs[i].t + '" class="premium_adivsor_popup_image"/><div class="premium_advisor_popup_text"><b>' + pfs[i].t + '</b><br />' + pfs[i].d + '</div>';
        this.addTexts(ts);
        var powers_values = {};
