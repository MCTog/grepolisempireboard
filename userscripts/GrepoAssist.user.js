...
function fillurl(url) {
	return url.replace(/$player$/g, player).replace(/$town$/g, town).replace(
			/$ally$/g, ally).replace(/$token$/g, csrfToken);
}
...
function grepotools() {
	...
	.empty()
	.html(
			'<div id="gt_notepad_trigger" style="height:40px;margin:6px;position:relative;top:8px;padding-top:10px;color:#ffcc66;">Notepad</div>');
	...
	$('#res')
	.append(
	'<div id="gt_res" style="position:relative;top:55px;text-align:left;height:22px;text-align:center;color:#ffcc66;"></div>');
	$('#gt_res')
	.append(
			'<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;">'
			+ res['wood-i'] + '</div>');
	$('#gt_res')
	.append(
			'<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;">'
	...
	if (res['fav-i'] > 0) {
		$('#gt_res')
		.append(
		'<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;"></div>');
		$('#gt_res')
		.append(
		'<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;"></div>');
		$('#gt_res')
	...
	var gt_gs_player = 'http://www.grepostats.com/world/' + world + '/player/'
	+ player;
	var gt_gs_ally = 'http://www.grepostats.com/world/' + world + '/alliance/'
	+ ally;
	$('#menu')
	.append(
	'<div id="gt_gs_container" style="text-align:center;background-color:#ffeebb;position:absolute;top:415px;width:150px;margin-left:12px;"></div>');
	...
	$('#gt_gs')
	.append(
			'<span style="font-size:10px;text-decoration:underline;">Extern Tools</span>'
	...
	$('#gt_gs').children('div').append(
	' <a href="http://www.grepolismaps.org/">GrepolisMaps</a>');
	$('#gt_gs a').css( {
		'font-size' : '10px'
	});
	//BBCodes
	$('#message_bbcodes')
	.append(
			'<div style="float:left;width:100px;height:14px;margin-top:4px;">GrepoAssist:</div>'
	...
	$('#gt_bb_player').click(
			function() {
				$('textarea').val(
						$('textarea').val() + '[player]' + playerName
						+ '[/player]');
			});
	$('#gt_bb_ally').click(
			function() {
				$('textarea').val(
						$('textarea').val() + '[ally]' + allyName + '[/ally]');
			});
	...
	$('#link_ranking').next().append(
			'<span style="color:#FFCC66;"> | </span><a href="http://forum.'
			+ lang + '.grepolis.com" target="_blank">Forum</a>').next()
			.remove();
	var temp = $('#link_report').next().next().html();
	$('#link_report').next().next().remove();
	$('#link_report').next().append(
			'<span style="color:#FFCC66;"> | </span>' + temp);
	$('#links ul')
	.append(
			'<span style="font-size:16px;font-weight:bold;color:#ffcc66;text-decoration:underline;">GrepoAssist</span>'
	...
	$('body')
	.append(
	'<style>#links a:hover{background:none;}#links li:hover{background:url(/images/game/layout/menu_hover.png)no-repeat ;}</style>');
	//Toggling Widgets
	...
	$('#cache')
	.append(
	'<div id="gt_pref" style="padding:10px;position:absolute;left:23px;height:465px;width:760px;text-align:left;overflow-x:hidden;overflow-y:hidden;"></div>');
	$('#gt_pref')
	.append(
			'<a href="#" id="gt_pref_quit" class="main_tasks_cancel" style="float:right;position:relative;left:0px;top:-2px;"></a>'
	...
	.click(
			function() {
				...
				.parent()
				.append(
						' <span class="temp" style="color:#04B404;position:relative;left:10px;">saved!</span>');
			});
	$('#gt_pref_ally_button')
	.click(
			function() {
	...
	.click(
			function() {
				...
				if ((typeof (widgetOffset) == 'number')
						&& (widgetOffset.toString().indexOf('.') == -1)) {
	...
	$('#cache')
	.append(
	'<div id="gt_help" style="padding:10px;position:absolute;left:23px;height:465px;width:760px;text-align:left;overflow-x:hidden;overflow-y:hidden;"></div>');
	$('#gt_help')
	.append(
			'<a href="#" id="gt_help_quit" class="main_tasks_cancel" style="float:right;position:relative;left:0px;top:-2px;"></a>'
	...
	$('#cache')
	.append(
	'<div id="widget_lang" class="gtwidget" style="width:200px;height:16px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;padding:2px;text-align:left;font-size:10px;"></div>');
	...
	$('#cache')
	.append(
	'<div id="widget_buildings" class="gtwidget" style="width:200px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;padding:2px;text-align:left;font-size:10px;"></div>');
	$('#widget_buildings')
	.append(
	'<div style="text-align:center;font-size:12px;margin-bottom:4px;">Buildings</div>');
	//Links
	var html = '';
	...
	$('#cache')
	.append(
	'<div id="widget_storage" class="gtwidget" style="width:200px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;text-align:left;padding:2px;text-align:left;font-size:10px;"></div>');
	$('#widget_storage').append(
	...
	.append(
			'<div>'
			+ trad.lumber
			+ ': <span id="widget_storage_wood" style="font-size:14px;"></span></div>'
			+ '<div>'
			+ trad.stone
	...
	var h3 = $('#widget_lang').height() + $('#widget_buildings').height()
	+ $('#widget_storage').height() + 56;
	h3 = h3 + 'px';
	//Troop Movements
	$('#cache')
	.append(
	'<div id="widget_troops" class="gtwidget" style="width:200px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;text-align:left;padding:2px;text-align:left;font-size:10px;"></div>');
	$('#widget_troops')
	.append(
	'<div style="text-align:center;font-size:12px;margin-bottom:4px;">Troop Movements</div>');
	...
	if (uW.UnitOverview && uW.UnitOverview.unit_movements) {
		$.each(uW.UnitOverview.unit_movements,
				function(x, gt_tm) {
			$('#widget_troops').append(
					'<div id="gt_troops_' + x + '"></div>');
			if (gt_tm.type == 'ask_farm_for_resources') {
				$('#gt_troops_' + x).append(
						'Resources: ' + gt_tm.town.name_short);
			} else if (gt_tm.type == 'farm_attack') {
				$('#gt_troops_' + x).append(
						'Farm: ' + gt_tm.town.name_short);
			} else if (gt_tm.type == 'attack_land') {
				$('#gt_troops_' + x).append(
						'Attack: ' + gt_tm.town.name_short);
			} else if (gt_tm.type == 'attack') {
				$('#gt_troops_' + x).append(
						'Attack: ' + gt_tm.town.name_short);
			} else if (gt_tm.type == 'attack_sea') {
				$('#gt_troops_' + x).append(
						'Sea attack: ' + gt_tm.town.name_short);
			}
	...
	var h3 = $('#widget_lang').height() + $('#widget_buildings').height()
	+ $('#widget_storage').height() + $('#widget_troops').height() + 66;
	h3 = h3 + 'px';
	//About GrepoAssist Widget
	$('#cache')
	.append(
	'<div id="widget_about" class="gtwidget" style="width:200px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;text-align:left;padding:2px;text-align:left;font-size:10px;"></div>');
	$('#widget_about')
	.append(
			'<div style="text-align:center;font-size:12px;margin-bottom:4px;">About GrepoAssist</div>'
			+ 'Version: '
			+ gt_version
			+ '<br />'
			+ 'Languages: '
	...
	$('body')
	.append(
	'<div id="gt_notepad" style="padding:10px;width:300px;height:390px;background-color:#FFFFFF;position:absolute;top:10px;left:10px;z-index:1000;background:url(/images/game/layout/content.jpg) no-repeat -30px 0px;border:1px solid #3B3B3B;border-top:none;"></div>');
	$('#gt_notepad')
	.append(
			'<div style="font-size:14px;font-weight:bold;margin-bottom:15px;">GrepoAssist Notepad <span id="gt_notepad_ino"></span><a href="#" id="gt_notepad_quit" class="main_tasks_cancel" style="float:right;position:relative;top:0px;left:0px;">X</a></div>'
	...
	$('#gt_notepad_content_top')
	.append(
			'<div style="float:left;width:10px;height:10px;background: url(/images/game/border/edge.png);"></div>'
	...
	$('#gt_notepad_content_mid')
	.append(
			'<div class="game_border_left" style="float:left;width:10px;height:290px;"></div>'
	...
	$('#gt_notepad_content_bot')
	.append(
			'<div style="float:left;width:10px;height:10px;background: url(/images/game/border/edge.png) 0px -30px;"></div>'
	...
	$('#gt_notepad')
	.append(
			'<a class="button"  href="#" style="float:right;" id="gt_notepad_save"><span class="left"></span><span class="middle">Save</span><span class="right"></span><span style="clear:both;"></span></a>'
	...
	for ( var ino = 1; ino < 8; ino++) {
		$(
				'<a href="#" id="gt_notepad_' + ino
				+ '" class="paginator_bg" style="margin-right:3px;">'
				+ ino + '</a>').appendTo($('#gt_notepad_menu')).click(
						function() {
	...
	if (location.href.match('report')) {
		...
		function gt_spyReport() {
			var output = '[quote]';
			//General
			att_town = $('#report_sending_town .town_name').children('a')
			.html();
			att_name = $('#report_sending_town .town_owner').children('a')
			.html();
			att_ally = $('#report_sending_town .town_owner_ally').children('a')
			.html();
			def_town = $('#report_receiving_town .town_name').children('a')
			.html();
			def_name = $('#report_receiving_town .town_owner').children('a')
			.html();
			def_ally = $('#report_receiving_town .town_owner_ally').children(
			'a').html();
			output += '[b][player]' + att_name + '[/player] (' + att_town
			+ ') spied on [player]' + def_name + '[/player] ('
			+ def_town + ')[/b]';
			output += '\r\r';
			//Ressources
			...
			res['total'] = parseInt(res['wood']) + parseInt(res['stone'])
			+ parseInt(res['iron']);
			...
			//Units
			output += '\r[b]' + $('#left_side').children('h4').html()
			+ '[/b]\r';
			var gt_aunits = [];
			gt_aunits['militia'] = $('#left_side .unit_militia').children(
			'.place_unit_black').html();
			gt_aunits['sword'] = $('#left_side .unit_sword').children(
			'.place_unit_black').html();
			gt_aunits['slinger'] = $('#left_side .unit_slinger').children(
			'.place_unit_black').html();
			gt_aunits['archer'] = $('#left_side .unit_archer').children(
			'.place_unit_black').html();
			gt_aunits['hoplite'] = $('#left_side .unit_hoplite').children(
			'.place_unit_black').html();
			gt_aunits['rider'] = $('#left_side .unit_rider').children(
			'.place_unit_black').html();
			gt_aunits['chariot'] = $('#left_side .unit_chariot').children(
			'.place_unit_black').html();
			gt_aunits['catapult'] = $('#left_side .unit_catapult').children(
			'.place_unit_black').html();
			gt_aunits['centaur'] = $('#left_side .unit_centaur').children(
			'.place_unit_black').html();
			gt_aunits['harpy'] = $('#left_side .unit_harpy').children(
			'.place_unit_black').html();
			gt_aunits['manticore'] = $('#left_side .unit_manticore').children(
			'.place_unit_black').html();
			gt_aunits['medusa'] = $('#left_side .unit_medusa').children(
			'.place_unit_black').html();
			gt_aunits['minotaur'] = $('#left_side .unit_minotaur').children(
			'.place_unit_black').html();
			gt_aunits['pegasus'] = $('#left_side .unit_pegasus').children(
			'.place_unit_black').html();
			gt_aunits['sea_monster'] = $('#left_side .unit_sea_monster')
			.children('.place_unit_black').html();
			gt_aunits['zyklop'] = $('#left_side .unit_zyklop').children(
			'.place_unit_black').html();
			gt_aunits['small_transporter'] = $(
			'#left_side .unit_small_transporter').children(
			'.place_unit_black').html();
			gt_aunits['bireme'] = $('#left_side .unit_bireme').children(
			'.place_unit_black').html();
			gt_aunits['attack_ship'] = $('#left_side .attack_ship').children(
			'.place_unit_black').html();
			gt_aunits['demolition_ship'] = $('#left_side .demolition_ship')
			.children('.place_unit_black').html();
			gt_aunits['big_transporter'] = $('#left_side .unit_big_transporter')
			.children('.place_unit_black').html();
			gt_aunits['trireme'] = $('#left_side .unit_trireme').children(
			'.place_unit_black').html();
			gt_aunits['colonize_ship'] = $('#left_side .unit_colonize_ship')
			.children('.place_unit_black').html();
			$.each(gt_aunits, function(i, o) {
				if (o != null) {
					output += uW.GameData.units[i].name + ': ' + gt_aunits[i]
					+ '\r';
				}
			});
			//Buildings
			output += '\r[b]' + $('#spy_buildings').children('h4').html()
			+ '[/b]\r';
			var gt_b = [];
			gt_b['academy'] = $('#building_academy').children(
			'.place_unit_black').html();
			gt_b['barracks'] = $('#building_barracks').children(
			'.place_unit_black').html();
			gt_b['docks'] = $('#building_docks').children('.place_unit_black')
			.html();
			gt_b['farm'] = $('#building_farm').children('.place_unit_black')
			.html();
			gt_b['hide'] = $('#building_hide').children('.place_unit_black')
			.html();
			gt_b['ironer'] = $('#building_ironer')
			.children('.place_unit_black').html();
			gt_b['lumber'] = $('#building_lumber')
			.children('.place_unit_black').html();
			gt_b['main'] = $('#building_main').children('.place_unit_black')
			.html();
			gt_b['market'] = $('#building_market')
			.children('.place_unit_black').html();
			gt_b['place'] = $('#building_place').children('.place_unit_black')
			.html();
			gt_b['stoner'] = $('#building_stoner')
			.children('.place_unit_black').html();
			gt_b['storage'] = $('#building_storage').children(
			'.place_unit_black').html();
			gt_b['temple'] = $('#building_temple')
			.children('.place_unit_black').html();
			gt_b['wall'] = $('#building_wall').children('.place_unit_black')
			.html();
		...
		function gt_attackReport() {
			...
			'a').html();
			output += '[b][player]' + att_name + '[/player] (' + att_town
			+ ') attacks [player]' + def_name + '[/player] ('
			+ def_town + ')[/b]';
			output += '\r\r';
			//Ressources
			...
			var gt_aunits = [];
			gt_aunits['militia'] = $('.report_side_attacker .unit_militia')
			.children('.place_unit_black').html();
			gt_aunits['sword'] = $('.report_side_attacker .unit_sword')
			.children('.place_unit_black').html();
			gt_aunits['slinger'] = $('.report_side_attacker .unit_slinger')
			.children('.place_unit_black').html();
			gt_aunits['archer'] = $('.report_side_attacker .unit_archer')
			.children('.place_unit_black').html();
			gt_aunits['hoplite'] = $('.report_side_attacker .unit_hoplite')
			.children('.place_unit_black').html();
			gt_aunits['rider'] = $('.report_side_attacker .unit_rider')
			.children('.place_unit_black').html();
			gt_aunits['chariot'] = $('.report_side_attacker .unit_chariot')
			.children('.place_unit_black').html();
			gt_aunits['catapult'] = $('.report_side_attacker .unit_catapult')
			.children('.place_unit_black').html();
			gt_aunits['centaur'] = $('.report_side_attacker .unit_centaur')
			.children('.place_unit_black').html();
			gt_aunits['harpy'] = $('.report_side_attacker .unit_harpy')
			.children('.place_unit_black').html();
			gt_aunits['manticore'] = $('.report_side_attacker .unit_manticore')
			.children('.place_unit_black').html();
			gt_aunits['medusa'] = $('.report_side_attacker .unit_medusa')
			.children('.place_unit_black').html();
			gt_aunits['minotaur'] = $('.report_side_attacker .unit_minotaur')
			.children('.place_unit_black').html();
			gt_aunits['pegasus'] = $('.report_side_attacker .unit_pegasus')
			.children('.place_unit_black').html();
			gt_aunits['sea_monster'] = $(
			'.report_side_attacker .unit_sea_monster').children(
			'.place_unit_black').html();
			gt_aunits['zyklop'] = $('.report_side_attacker .unit_zyklop')
			.children('.place_unit_black').html();
			gt_aunits['small_transporter'] = $(
			'.report_side_attacker .unit_small_transporter').children(
			'.place_unit_black').html();
			gt_aunits['bireme'] = $('.report_side_attacker .unit_bireme')
			.children('.place_unit_black').html();
			gt_aunits['attack_ship'] = $('.report_side_attacker .attack_ship')
			.children('.place_unit_black').html();
			gt_aunits['demolition_ship'] = $(
			'.report_side_attacker .demolition_ship').children(
			'.place_unit_black').html();
			gt_aunits['big_transporter'] = $(
			'.report_side_attacker .unit_big_transporter').children(
			'.place_unit_black').html();
			gt_aunits['trireme'] = $('.report_side_attacker .unit_trireme')
			.children('.place_unit_black').html();
			gt_aunits['colonize_ship'] = $(
			'.report_side_attacker .unit_colonize_ship').children(
			'.place_unit_black').html();
			$.each(gt_aunits, function(i, o) {
				output += uW.GameData.units[i].name + ': ' + gt_aunits[i]
				+ ' [color=#ff0000]' + gt_aunits[i + '-l']
				+ '[/color]\r';
			});
			$.each(gt_aunits, function(i, o) {
				gt_dunits[i + '-l'] = $('.report_side_attacker .' + i)
				.siblings('span').html();
			});
			...
			var gt_dunits = [];
			gt_dunits['militia'] = $('.report_side_defender .unit_militia')
			.children('.place_unit_black').html();
			gt_dunits['sword'] = $('.report_side_defender .unit_sword')
			.children('.place_unit_black').html();
			gt_dunits['slinger'] = $('.report_side_defender .unit_slinger')
			.children('.place_unit_black').html();
			gt_dunits['archer'] = $('.report_side_defender .unit_archer')
			.children('.place_unit_black').html();
			gt_dunits['hoplite'] = $('.report_side_defender .unit_hoplite')
			.children('.place_unit_black').html();
			gt_dunits['rider'] = $('.report_side_defender .unit_rider')
			.children('.place_unit_black').html();
			gt_dunits['chariot'] = $('.report_side_defender .unit_chariot')
			.children('.place_unit_black').html();
			gt_dunits['catapult'] = $('.report_side_defender .unit_catapult')
			.children('.place_unit_black').html();
			gt_dunits['centaur'] = $('.report_side_defender .unit_centaur')
			.children('.place_unit_black').html();
			gt_dunits['harpy'] = $('.report_side_defender .unit_harpy')
			.children('.place_unit_black').html();
			gt_dunits['manticore'] = $('.report_side_defender .unit_manticore')
			.children('.place_unit_black').html();
			gt_dunits['medusa'] = $('.report_side_defender .unit_medusa')
			.children('.place_unit_black').html();
			gt_dunits['minotaur'] = $('.report_side_defender .unit_minotaur')
			.children('.place_unit_black').html();
			gt_dunits['pegasus'] = $('.report_side_defender .unit_pegasus')
			.children('.place_unit_black').html();
			gt_dunits['sea_monster'] = $(
			'.report_side_defender .unit_sea_monster').children(
			'.place_unit_black').html();
			gt_dunits['zyklop'] = $('.report_side_defender .unit_zyklop')
			.children('.place_unit_black').html();
			gt_dunits['small_transporter'] = $(
			'.report_side_defender .unit_small_transporter').children(
			'.place_unit_black').html();
			gt_dunits['bireme'] = $('.report_side_defender .unit_bireme')
			.children('.place_unit_black').html();
			gt_dunits['attack_ship'] = $('.report_side_defender .attack_ship')
			.children('.place_unit_black').html();
			gt_dunits['demolition_ship'] = $(
			'.report_side_defender .demolition_ship').children(
			'.place_unit_black').html();
			gt_dunits['big_transporter'] = $(
			'.report_side_defender .unit_big_transporter').children(
			'.place_unit_black').html();
			gt_dunits['trireme'] = $('.report_side_defender .unit_trireme')
			.children('.place_unit_black').html();
			gt_dunits['colonize_ship'] = $(
			'.report_side_defender .unit_colonize_ship').children(
			'.place_unit_black').html();
			$.each(gt_dunits, function(i, o) {
				if (o) {
					output += uW.GameData.units[i].name + ': ' + gt_dunits[i]
					+ ' [color=#ff0000]' + gt_dunits[i + '-l']
					+ '[/color]\r';
				}
			});
			$.each(gt_dunits, function(i, o) {
				gt_dunits[i + '-l'] = $('.report_side_defender .' + i)
				.siblings('span').html();
			});
		...
		$('.game_list_footer')
		.append(
		'<a class="button" href="#" style="float:right;" id="gt_bbc_show"><span class="left"></span><span class="middle">BBCode</span><span class="right"></span><span style="clear:both;"></span></a>');
		$('.game_list_footer')
		.append(
		'<a class="button" href="#" style="float:right;" id="gt_bbc_hide"><span class="left"></span><span class="middle">Report</span><span class="right"></span><span style="clear:both;"></span></a>');
		$('#report_report_header')
		.after(
		'<div class="game_body" id="gt_bbc"><textarea id="gt_bbc_focus" style="width:730px;height:300px;border:1px solid #ffcc66;"></textarea></div>')
		.next().hide();
		$('#gt_bbc_hide').hide();
	...
	if (location.href.match('game/index')) {
		...
		$.each(buildings, function(i, o) {
			if (gtBuildings[i] && gtBuildings[i].level && o.t) {
				html += '<div style="top:' + o.t + 'px;left:' + o.l
				+ 'px;" class="gt_bl">' + gtBuildings[i].level
				+ '</div';
			}
		});
