...
if( typeof faarksErrorReporter === "undefined" ){
			...
			initCss: function( ){
				if( $("#faarksErrorReporter_styles").length <= 0 ){
					$("head").append("<style type='text/css' id='faarksErrorReporter_styles'>\
						#faarksErrorReporter_overlay{ display: block; height: 100%; left: 0; position: fixed; top: 0; width: 100%; z-index: 40000; opacity: 0.4; background-image:url(http://grepo.faark.de/faarksInternalToolset/rel/curtain_bg.png); }\
	...
	return;
			...
			try{
				...
				var file,line,stack,msg,type="Direct";
				...
				if( (typeof e === "object") && (typeof e.message === "string") && (typeof e.fileName === "string") && (!isNaN( Number( e.lineNumber ) ) ) && (typeof e.stack === "string")){//exception
					...
					//if( typeof console === "object") console.log("Reporting error... Arugments: %o",arguments);
					var bestProduct = this.i_get_best_package( arguments[0], arguments[1], arguments[2], arguments[3] );
				}else
				...
				var bestPackage = null,
					locD = this.i_unpack_stack( new Error().stack, 2 ),
				...
				if( window.dev === true ){// on my dev version, make a "alert" for every catched exception...
...
try{
	...
	if( (typeof faarksTextSelection === "undefined") || (faarksTextSelection.version < 2)){
		var faarksTextSelection = (function(){
			var obj = {
				...
				init: function( sysname, name, a, b, c ){
					if( arguments.length == 0 ){//migrate old data
						if( (typeof faarksTextSelection !== "undefined") && (faarksTextSelection !== this ) ){
							for( var p in faarksTextSelection.projects ){
								if( !this.projects[p] ){
									this.projects[p] = faarksTextSelection.projects[p];
								}else{
									//copy missing replacements
...
//			moralInPopup_townInfo_moral_popup: "Diese Moral wirst du vorraussichtlich haben,<br/>wenn du diese Stadt angreiffst.<br/><br/>Aber wie üblich sind alle Angaben ohne Gewähr,<br/>va. kürzliche Änderungen kennt der<br/>Grex's Moral-Server vl noch nicht drin.[[BY]]",
		...
		en: {
			...
			moralInPopup_townInfo_moral_popup: "That is the moral, u should have when<br/>attacking that town. No liability assumed! ;)<br/>[[BY]]"
		},
		fr: {
			townid_text: " (Id: [[TOWNID]])",
	...
	if( typeof faarksUtility === "undefined" ){
		var faarksUtility = {
	...
	if( (typeof faarksDataStorage === "undefined") ){
		//requires faarksUtility
		var faarksDataStorage = {
			config: {
				last_etag: null,
				default_login: null,
				lastupdatetime: null,
				weblocation: "http://grepo.faark.de/faarksDataStorage/v0001.php",
				initiated: false,
				default_options: {
					context: function(){throw new Error("No default context set!");},
					onupdate: null, // function( newdata )
					updateevery: 30, //seconds to refresh content from server or chace,
					waitafterupdate: 10, //seconds to wait after data was set, to not update to often
					callback_on_init_suppress: false, // data for a project can be aviable before calling init("mystuff"...). by default, the callback is called in that case...
					callback_on_init_default_too: false // [needs callback_on_init_suppress=false], calls callback on init in every case, also if only default data is aviable...
				},
				default_status: {
					uploaded: null, // date of the last data-upload
					changed: null, // date data has changed and needs upload
					downloaded: null // date of the last download (or cache)
				},
				onupdated: []
			},
			logins: {/* ONLY ONE ALLOWED ATM
				userident: {
					identification: text,
					authentication: text,
					cachekey: text //atm in config.default_login
				}
			*/},
			projects: {/*
				name: {
					...
					shared: [],
					my_shared: object,
					options: //SEE config.default_options
					status: //See config.default_status
					packageShortcut: shortcut object to that data ( use obj.get( "index" ) instead of fds.get( "mypack", "index" ); can be downward compatible to future versions )
				}
			*/},
			observers: {
				cookie: { // reads the cookie every ... to check for a new etag/key by e.g. another tab
					intervalObject: null,
					intervalTime: 10000,
					reread: function(){ //called every 10s to reread cookies
						var etag = faarksUtility.cookie.get( "fds_etag" );
						if( etag && (etag != faarksDataStorage.config.last_etag) ){
							faarksDataStorage.config.last_etag = etag;
							faarksDataStorage.observers.onlinedata.request();
						}
					},
					init: function(){
						var aviAuth = faarksUtility.cookie.get( "fds_auth" );
						if( aviAuth && (aviAuth = /([^:]+):([^\s]+)/) ){
							faarksDataStorage.logins[ aviAuth[1] ] = faarksDataStorage.i_normalizeLogin( { u: aviAuth[1], p: aviAuth[2] } );
						}
						this.intervalObject = window.setInterval( fer_cf( this.reread, this ), this.intervalTime );
					}
				},
				onlinedata: { // reads online(/cached)-data every...
					onresponde: function( data, request, xhr, status, error ){
						if( error || !data ){
							HumanMessage.error( "error in fds: "+data+"/"+error);
							//faarksDataStorage.logins[faarksDataStorage.config.default_login]
						}else{
							if( data.etag ){
								faarksDataStorage.config.last_etag = data.etag;
								faarksUtility.cookie.set( "fds_etag", data.etag );
							}
							if( data.error ){
							...
							var ds = {};
							if( data.packages ){
								for( var i = 0; i < data.packages.length; i++ ){ // create a indexd array
									if( data.packages[i].context === faarksDataStorage.config.default_options.context() ){
										try{
											ps[ data.packages[i]["package"] ] = $.secureEvalJSON( data.packages[i].content );
										}catch( whatever ){
											ps[ data.packages[i]["package"] ] = data.packages[i].content;
										}
									}
								}
							}
							if( data.shared ){
								for( var i = 0; i < data.shared.length; i++ ){
									if( data.shared[i].context === faarksDataStorage.config.default_options.context() ){
										var sd = data.shared[i];
										if( !ds[ sd["package"] ] ){
											ds[ sd["package"] ] = [];
										}
										try{
											sd.data =  $.secureEvalJSON( sd.data );
										}catch(err){}
										ds[ data.shared[i]["package"] ].push( sd );
									}
								}
							}
							if( data.packages || data.shared ){
								for( var i in faarksDataStorage.projects ){ // update all local packages
									faarksDataStorage.init( i, typeof ps[i] === "undefined" ? null : ps[i], null, null, { downloaded: new Date() }, typeof ds[i] === "undefined" ? null : ds[i] );
								}
								for( var i = 0; i < faarksDataStorage.config.onupdated.length; i++ ){
									fer_call( faarksDataStorage.config.onupdated[i], this, arguments );
								}
							}
						}
					},
					request: function(){
						...
						$.ajax({
							cache: true,
							url: faarksDataStorage.i_getRequestAddress(),
							success: fer_cf( function( data, textStatus, xhr ){
								self.onresponde( data, this, xhr, textStatus );
							}),
							error: fer_cf( function( xhr, textStatus, errorThrown ){
								self.onresponde( null, this, xhr, textStatus, errorThrown );
							}),
							dataType: "json"
						});
					},
					init: function(){
						window.setTimeout( fer_cf( this.request, this ), 100 );
					}
				},
				localdata: { // checks lokal data for updates and uploads...
					onresponde: function( data, request, xhr, status, error ){
						if( error ){
							for( var i = 0; i < request.updated_packages.length; i++ ){
								faarksDataStorage.projects[ request.updated_packages[i] ].status.uploaded = null;
							}
						}else{
							faarksDataStorage.observers.onlinedata.onresponde( data, request, xhr, status, error );
						}
						if( request.special_callback ){
							request.special_callback( data, request, xhr, status, error );
						}
					},
					upload: function( projectsToUpdate, additionalCallback ){
						return;//DS-OFF
						
						if( !(projectsToUpdate instanceof Array) )
						...
						for( var i = 0; i < projectsToUpdate.length; i++ ){
							var p = faarksDataStorage.projects[ projectsToUpdate[i] ];
							var d = { name:p.name, data: JSON.stringify( p.data ), context: p.options.context() };
							if( p.my_shared )
								d.shared = JSON.stringify( p.my_shared );
							data.packages.push( d );//json-cl seri
//							data.packages.push( {name: p.name, data: p.data } ); //php seri
						}
						var self = this;
						$.ajax({
							type: 'POST',
							updated_packages: projectsToUpdate,
							special_callback: additionalCallback,
							url: faarksDataStorage.i_getRequestAddress(),
							data: data,
							success: fer_cf( function( data, textStatus, xhr ){
								self.onresponde( data, this, xhr, textStatus );
						...
						for( var i = 0; i < projectsToUpdate.length; i++ ){// early version can have a lot of errors, so do that after ajax
							faarksDataStorage.projects[ projectsToUpdate[i] ].status.uploaded = new Date();
						}
					},
					check: function( force ){
						var updateNow = false; //set true to do a upload (changed data can be in waiting-state, so no upload for that)
						var dataThatNeedsUpdate = []; //all changed data, upload that at once
						for( var i in faarksDataStorage.projects ){
							var p = faarksDataStorage.projects[i];
							if(p.status.changed > p.status.downloaded ){
								dataThatNeedsUpdate.push( p.name );
								updateNow = updateNow || (p.status.changed > p.status.uploaded);//dont force when upload in progress
							}
						}
						if( force || ( updateNow && (dataThatNeedsUpdate.length > 0) ) )
							this.upload( dataThatNeedsUpdate );
					},
					start: function(){
					},
					init: function(){
			...
			i_getProject: function( pack ){
				if( this.projects[pack] ){
					return this.projects[pack];
				}else
					throw new Error("No Project with name ["+pack+"] registred");
			},
			i_isDifferent: function( a, b, strict ){
				if( typeof a === typeof b ){
					switch( typeof a ){
						...
						case "object":
							...
							for(var i in a){
								cnt++;
								if( !this.i_isDifferent( a[i], b[i], strict ) )
									return false;
							}
							for(var i in b){
								cnt--;
							}
							return cnt == 0;//same element numbers
						case "undefined":
							return true;
					}
				}else if( strict )
					...
					if( ((typeof a === "function") && (b == null)) || ((typeof b === "function") && (a == null)) )// not strict -> Ignor Function vs Null
						return true;
					return a != b;
				}
			},
			i_getRequestAddress: function( login_identification ){
				if( !this.config.default_login ){
					var list = [];
					var best = null;
					for( var i in this.logins ){
						list.push( this.logins[i] );
						if( "abcdefghijklmnopqrstuvwxyz".indexOf( i.substr( 0, 1 ) ) >= 0 ){ //1st char in a-z => real acc not session/tmp
							best = i;
						}
					}
					if( list.length > 0 ){
						if( !best ){
							best = list[Math.floor( Math.random() * list.length )].identification;
						}
						this.config.default_login = best;
					}
				}
				if( typeof this.logins[ login_identification ] === "undefined" )
					login_identification = this.config.default_login;
				var l = this.logins[login_identification];
				if( !l )
					return this.config.weblocation + "/_hello";
				return this.config.weblocation + "/call/" + l.identification + "/" + l.authentication;
			},
			i_createGetShortcut: function( namespace ){
				var self = this;
				return {
					get: function(a,b,c,d,e){return self.get(namespace,a,b,c,d,e);},
					store: function(a,b,c,d,e){return self.store(namespace,a,b,c,d,e);},
					data: function(a,b,c,d,e){return self.data(namespace,a,b,c,d,e);},
					shared: function(a,b,c,d,e){return self.shared(namespace,a,b,c,d,e);}
				}
			},
			i_normalizeLogin: function( user_login ){
				var el = {};
				if( typeof user_login.name !== "undefined" )
					el.identification = user_login.name;
				else if( typeof user_login.un !== "undefined" )
					el.identification = user_login.un;
				else if( typeof user_login.n !== "undefined" )
					el.identification = user_login.n;
				else if( typeof user_login.u !== "undefined" )
					el.identification = user_login.u;
				else if( typeof user_login.userident !== "undefined" )
					el.identification = user_login.userident;
				else if( typeof user_login.identification !== "undefined" )
					el.identification = user_login.identification;
				else
					throw new Error( "Invalid User-Login... no identification given" );
					
				if( typeof user_login.password !== "undefined" )
					el.authentication = user_login.password;
				else if( typeof user_login.pass !== "undefined" )
					el.authentication = user_login.pass;
				else if( typeof user_login.pw !== "undefined" )
					el.authentication = user_login.pw;
				else if( typeof user_login.p !== "undefined" )
					el.authentication = user_login.p;
				else if( typeof user_login.authentication !== "undefined" )
					el.authentication = user_login.authentication;
				else
					throw new Error( "Invalid User-Login... no authentication given" );
					
				return el;
			},
			shared: function( pack, data ){
				var p = this.i_getProject( pack );
				if( data ){
					p.my_shared = data;
					p.status.changed = new Date();
					this.observers.localdata.start();
				}
				return p.shared;
			},
			get: function( pack, index ){
				var p = this.i_getProject( pack );
				if( index || (typeof p.data[index] !== "undefined" ))
					return p.data[index];
				else
					return p.data;
			},
			store: function( pack, data, index ){
				var p = this.i_getProject( pack );
				var gotIndex = index || (typeof p.data[index] !== "undefined" );
				/*if( gotIndex && !data ){
					p.data = {};
				}*/
				if( this.i_isDifferent( data, gotIndex ? p.data[index] : p.data ) ){
					p.status.changed = new Date();
					this.observers.localdata.start();
				}
				if( gotIndex )
			...
			data: function( pack, index, data ){
				if( (arguments.length >= 3) || (typeof index === "object") ){
					this.store( pack, data, index );
					return data;
				}else{
					return this.get( pack, index );
				}
			},
			save: function( callback ){//forces saving the current data, dont use that for automated stuff/without user interaction!
				//1. collect new data
				var projectsToUpdate = [];
				for( var i in this.projects ){
					if( (this.projects[i].status.changed > this.projects[i].status.uploaded) || (this.projects[i].status.changed > this.projects[i].status.downloaded) ){
						projectsToUpdate.push( i );
					}
				}
				if( projectsToUpdate.length > 0 ){
					this.observers.localdata.upload( projectsToUpdate, callback );
				}else{
					callback();
				}
			},
			init: function( pack, default_data, user_login /*{name:...;password:...}*/, options, status, shared ){
				if( user_login ){ //got user-login, add that to logins (as default?)
					this.logins[ el.identification ] = this.i_normalizeLogin( user_login );
					//this.config.default_login = el.identification;
				}
				if( !this.config.initiated ){//default first init
					this.observers.cookie.init();
					this.observers.onlinedata.init();
					this.observers.localdata.init();
					if( (typeof faarksDataStorageLogins === "object") && (typeof faarksDataStorageLogins.length === "number") ){
						for( var i = 0; i < faarksDataStorageLogins.length; i++ ){
							var d = this.i_normalizeLogin( faarksDataStorageLogins[i] );
							this.logins[ d.identification ] = d;
						}
					}
					this.config.initiated = true;
				}
				if( arguments.length > 0 ){// add package
					var p;
					if( this.projects[pack] ){
						p = this.projects[pack];
						if( ((status.downloaded != null) || !p.status.downloaded) && default_data )//indicates new data
							p.data = default_data;
						if( ((status.downloaded != null) || !p.status.downloaded) && shared )//indicates new data
							p.shared = shared;
						p.options = $.extend( p.options, options );
						p.status = $.extend( p.status, status );
					}else{
						this.projects[pack] = p = {
							name: pack,
							data: typeof default_data === "undefined" ? {} : default_data,
							shared: typeof shared === "undefined" ? [] : shared,
							options: $.extend( {}, this.config.default_options, options ),
							status: $.extend( {}, this.config.default_status, status ),
							packageShortcut: this.i_createGetShortcut( pack )
						};
					}
					
					if( p.options.onupdate && ( (status && status.downloaded && ( ( !options || !options.onupdate ) || !p.options.callback_on_init_suppress )) || (!p.options.callback_on_init_suppress && p.options.callback_on_init_default_too) ) ){ // got callback && needCall=((new&(not init || call on init)) || (call always))
						p.options.onupdate( default_data, pack, p );
					}
					
					return p.packageShortcut;
				}
			}
		};
		/* The folowing class isnt done, jet! */
		faarksTextSelection.init("fgsc","Faarks Grepo Config-Tool", {
			de: {
				faarksDataStorage_config_headlineText: "Tool - Einstelunngen",
				faarksDataStorage_config_defaultItemContent: "Nothing found to configure",
				faarksDataStorage_config_saveButtonText: "Speichern",
				faarksDataStorage_config_cancelButtonText: "Abbrechen",
				faarksDataStorage_config_configButtonText: "Tools"
			}
		});
		var faarksGrepoSavedConfig = {
			/* members */
			is_edit_mode: false,
			content: [],
			...
			find: function( projectname ){
				for( var i = 0; i < content.length; i++ ){
					if( content[i].sysname.name == projectname )
						return content[i];
				}
				return null;
			},
			/* ci... : INTERNAL CLASSES! DONT USE THEM */
			ciVar: function( sysname, default_value /* if list(4 special-types), [0] is default */, describtion_object, type, special_type ){
				this.sysname = sysname;
				this.description = describtion_object;
				this.basic_type = type;
				this.special_type = special_type ? special_type : "default"; 
				this.default_value = faarksUtility.data.verify( default_value, this.basic_type );
				this.value = undefined;
				this.assigned_project = null;
				
				this.getName = function (){
					return this.sysname.name;
				};
				...
				this.set = function( value, dontSaveToDb ){
					this.value = faarksUtility.data.verify( value, this.basic_type );
					if( this.assigned_project && !dontSaveToDb ){
						this.assigned_project.storageInterface.store( faarksUtility.data.to_storable( value ), this.getName() );
					}
				};
				this.isSet = function(){
				...
				this.assign = function( project ){
					this.assigned_project = project;
				};
			},
			ciVarGroup: function( data ){
				var fgsc = faarksGrepoSavedConfig;
				
				//members
				this.sub_elements = [];
				this.description = null;
				this.drawable_childs = 0;
				
				//methods
				this.getVarsElements = function(){
					var l = [];
					for( var i = 0; i < this.sub_elements.length; i++ ){
						if( this.sub_elements[i] instanceof fgsc.ciVar ){
							l.push( this.sub_elements[i] );
						}else if( this.sub_elements[i] instanceof fgsc.ciVarGroup ){
							l = l.concat( this.sub_elements[i].getVarsElements() );
						}
					}
					return l;
				}
				...
				for( var i = 0; i < data.length; i++ ){
					if( data[i] instanceof fgsc.ciDescription ){
						if( !this.description )
							this.description = data[i];
						else
							throw new Error("VarGroup can have only 1 Describtion!");
					}else if( data[i] instanceof fgsc.ciVar ){
						this.sub_elements.push( data[i] );
					}else if( data[i] instanceof fgsc.ciVarGroup ){
						this.sub_elements.push( data[i] );
//					}else if( data[i] instanceof Array ){
//						this.sub_elements.push( new 
					}
				}
				//more init: clear up useless crap
				for( var i = 0; i < this.sub_elements.length; i++ ){
					if( this.sub_elements[i] instanceof faarksGrepoSavedConfig.ciVar ){
						if( this.sub_elements[i].description )
							this.drawable_childs++;
					}else if( this.sub_elements[i] instanceof faarksGrepoSavedConfig.ciVarGroup ){
						this.drawable_childs += this.sub_elements[i].drawable_childs;
						if( this.sub_elements[i].sub_elements.length == 0 )
							this.sub_elements.splice( i, 1 );
					}
				}
				if( (this.sub_elements.length == 1) && ( this.sub_elements[0] instanceof faarksGrepoSavedConfig.ciVarGroup ) ){
					if( !this.description || !this.sub_elements[0].description ){
						this.description = this.description ? this.description : this.sub_elements[0].description;
						this.sub_elements = this.sub_elements[0].sub_elements;
					}
				}
			},
			ciDescription: function( readable_name, description ){
				this.name = readable_name;
				this.description = description;
			},
			ciSysname: function( sysname ){
				this.allowedChars = "abcdefghijklmnopqrstuvwxyz01234567890_";
				this.name = "";
				this.get = function(){ return this.name };
				sysname = sysname.toLowerCase();
				for( var i = 0; i < sysname.length; i++ ){
					if( this.allowedChars.indexOf( sysname.substr( i, 1 ) ) >= 0 ){
						this.name += sysname.substr( i, 1 );
					}
				}
				if( !this.name ){
					throw new Error( "Invalid Sysname ["+arguments[0]+"]!");
				}
			},
			ciProject: function( data ){
				var fgsc = faarksGrepoSavedConfig;
				...
				this.var_group = null;
				this.var_list = [];
				this.indexed_var_list = {};
				this.onupdate_callback = null;
				this.storageInterface = null;
				
				//methods
				this.i_oncontentrecived = fer_cf(function( data ){
					if( data && (typeof data === "object") ){
						for( var i = 0; i < this.var_list.length; i++ ){
							var v = this.var_list[i];
							if( typeof data[ v.getName() ] !== "undefined" ){
								v.set( data[ v.getName() ], true );
							}
						}
					}//we dont need a "else"... "var" returns default value by default^^
					if( this.onupdate_callback )
						this.onupdate_callback.call( null, this, data );
				},this);
				this.find = function( index ){
					var el = this.indexed_var_list[index.toLowerCase()];
					if( !el )
						throw new Error("No property ["+index+"] found in ["+this.sysname.get()+"]");
					return el;
				};
				this.get = function( index ){
					return this.find(index).get();
				};
				this.set = function( index, value ){
					return this.find(index).set(value);
				};
				this.shared = function(){
					return this.storageInterface.shared();
				}
				...
				for( var i = 0; i < data.length; i++ ){
					var d = data[i];
					if( d instanceof fgsc.ciSysname ){
						if( !this.sysname )
							this.sysname = d;
						else
							throw new Error("Project can have only 1 Sysname!");
					}else if( d instanceof fgsc.ciDescription){
						if( !desc )
							desc.push( d );
						else
							throw new Error("Project can have only 1 Describtion!");
					}else if( d instanceof fgsc.ciVarGroup ){
						subs.push( d );
					}else if( d instanceof fgsc.ciVar ){
						subs.push( d );
					}else if( typeof d === "function" ){
						if( !this.onupdate_callback )
							this.onupdate_callback = d;
						else
							throw new Error("Project can have only 1 onUpdate-Callback!");
					}else{
						throw new Error("Unknown object ["+d+"]");
					}
				}

				this.var_group = new fgsc.ciVarGroup( subs );
				this.var_list = this.var_group.getVarsElements();
				
				if( !this.sysname || (this.var_list.length == 0) ){
					throw new Error("Projects requires sysname and at least on var!");
				}
				if( fgsc.find( this.sysname.name ) ){
					throw new Error("a Project with name ["+this.sysname.name+"] already exists!");
				}
				
				fgsc.content.push( this );
				
				var defaultData = {};
				for( var i = 0; i < this.var_list.length; i++ ){
					//defaultData[ this.var_list[i].getName() ] = faarksUtility.data.to_storable( this.var_list[i].getDefault() ); DONT STORE DEFAULT DATA, ATM
					this.indexed_var_list[ this.var_list[i].getName() ] = this.var_list[i];
					this.var_list[i].assign( this );
				}
				
				this.storageInterface = faarksDataStorage.init( this.sysname.name, defaultData, null, { onupdate: this.i_oncontentrecived } );
			},
			/* local classes */
			/* c... : Creates Objects of local classes to create the Settings */
			cVar1: function( sysname, default_value, type, description, specialType ){
				/* 
					specialType
						null => default
						"list" => a list of multiple items
						"select" => a item of coosable els
						"radio" => like select, but just 1
						"fix_radio"/"fix_select" => like radio/select, but no way to add...
						Anything else/regex => vaildait-expression
				*/
				if( typeof sysname === "string" )
					sysname = new this.ciSysname( sysname );
				if( typeof description === "string" )
					description = new this.ciDescription( description );
				if( type instanceof this.ciDescription ){
					description = type;
					type = null;
				}
				if( (typeof type === "undefined") || (type == null) )
					type = faarksUtility.data.get_type( default_value );
				
				return new this.ciVar( sysname, default_value, description, type, specialType );
			},
			cVarGroup1: function( description, values ){
				var args = ((arguments.length==1)&&(arguments[0] instanceof Array))?arguments[0]:arguments;
				var data = [];
				for( var i = 0; i < args.length; i++ ){
					if( typeof args[i] === "string" ){
						if( i == 0 ){
							data.push( new this.ciDescription( args[i] ) );
						}else{
							data.push( new this.ciVar( args[i] ) );
						}
					}else if( typeof args[i] === "object" ){
						if( args[i] instanceof Array ){
							if( args[i].length > 0 )
								data.push( this.cVarGroup1( args[i] ) );
						}else{
							data.push( args[i] );
						}
					}else
						throw new Error( "Invalid Param for group: ["+args[i]+"]");
				}
				return new this.ciVarGroup( data );
			},
			cDescription1: function( name, info ){
				return new this.ciDescription( name, info );
			},
			cSysname1: function( sysname ){
				return new this.ciSysname( sysname );
			},
			cProject1: function( projectsysname, projectdescription, varlist ){
				var defWay = true;
				var data = [];
				for( var i = 0; i < arguments.length; i++ ){
					if( typeof arguments[i] === "string" ){
						if( defWay && (i == 0) ){
							data.push( new this.ciSysname( arguments[i] ) );
						}else if( defWay && (i == 1) ){
							data.push( new this.ciDescription( arguments[i] ) );
						}else{
							data.push( new this.ciVar( arguments[i] ) );
						}
					}else if( typeof arguments[i] === "object" ){
						defWay = false;
						if( arguments[i] instanceof Array ){
							if( arguments[i].length > 0 ){
								data.push( this.cVarGroup1( arguments[i] ) );
							}
						}else{
							data.push( arguments[i] );
						}
					}else if( typeof arguments[i] === "function" ){ // 1 function as cb can be added!
						data.push( arguments[i] );
					}else{
						throw new Error( "Invalid argument["+i+"] given to faarksGrepoSavedConfig.init(...): ["+arguments[i]+"].");
					}
				}
				
				return new this.ciProject( data );
			},
			/* Interfaces */
			interfaces: {
				grepo: {
					...
					usedInterfaces: {},
					save: function(){
						for( var i = 0; i < this.usedInterfacesList.length; i++ ){
							var data = this.usedInterfaces[ this.usedInterfacesList[i] ];
							var p = data.project;
							for( var v = 0; v < p.var_list.length; v++ ){
								var n = p.var_list[v].getName();
								if( typeof data.changedData[n] !== "undefined" ){
									p.var_list[v].set( data.changedData[n] );
								}
							}
						}
						faarksDataStorage.save( function( data, request, xhr, status, error ){
							if( arguments.length > 0 ){
								if( !error && !data.error && !data.message )
									HumanMessage.success( "Config saved!" );
							}else{
								HumanMessage.error( "Nothing to save!" );
							}
						} );
						this.cancel();
					},
					cancel: function(){
						$("#faarksDataStorage_configListBox li").remove();
						this.usedInterfaces = {};
						this.usedInterfacesList = [];
						this.init();
					},
					drawBasicInterface: function(){
						$('head').append("<style type='text/css'>\
	#faarksDataStorage_configListBox {\
		height:300px;\
		overflow-y:auto;\
		overflow-x:hidden;\
		width: 744px;\
	}\
	#faarksDataStorage_configListBox li, #faarksDataStorage_configListBox div.groupBox{\
		padding-left:10px;\
	}\
	#faarksDataStorage_configListBox div.varBox{\
		margin:5px;\
	}\
	#faarksDataStorage_mainBox{\
		top:80px;\
		position:absolute;\
	}\
</style>");
						$("#player_settings, #settings_quickbar").after("<div class='game_inner_box' id='faarksDataStorage_mainBox'>\
	<table cellspacing='0' cellpadding='0' class='game_border'><tbody>\
		<tr><td class='game_border_edge'></td><td class='game_border_top'></td><td class='game_border_edge game_border_edge_2'></td></tr>\
		<tr>\
			<td class='game_border_left'></td>\
			<td style='z-index: 1; position: relative;'>\
				<div style='margin: 0px; position: relative; display: inline-block;'>\
					...
					<div id='settings_header' class='game_header bold'>"+faarksTextSelection.get("fgsc","faarksDataStorage_config_headlineText")+"</div>\
					<ul class='game_list' id='faarksDataStorage_configListBox'><li id='faarksDataStorage_configListEmptyElement'>"+faarksTextSelection.get("fgsc","faarksDataStorage_config_defaultItemContent")+"</li></ul>\
					<div class='game_footer'>\
						<a style='float: left;' onclick='return false;' href='#' id='faarksDataStorage_config_saveButton' class='button'>\
							<span class='left'><span class='right'><span class='middle'>"+faarksTextSelection.get("fgsc","faarksDataStorage_config_saveButtonText")+"</span></span></span>\
							<span style='clear: both;'></span>\
						</a>\
						<a style='float: left;' onclick='return false;' href='#' id='faarksDataStorage_config_cancelButton' class='button'>\
							<span class='left'><span class='right'><span class='middle'>"+faarksTextSelection.get("fgsc","faarksDataStorage_config_cancelButtonText")+"</span></span></span>\
							<span style='clear: both;'></span>\
						</a>\
					</div>\
				</div>\
		...
		<tr><td class='game_border_edge game_border_edge_3'></td><td class='game_border_bottom'></td><td class='game_border_edge game_border_edge_4'></td></tr>\
	</tbody></table>\
</div>");
						$("#player_settings, #settings_quickbar").remove();
						$("#faarksDataStorage_config_saveButton").click( fer_cf( this.save, this) );
						$("#faarksDataStorage_config_cancelButton").click( fer_cf( this.cancel, this) );
					},
					onChange: function( el, data ){
						this.usedInterfaces[ el.assigned_project.sysname.name ].changedData[el.getName()] = data;
					},
					getCheckbox: function( el ){
						var input = $('<input></input>');
						input.attr("name","fgsc["+el.getName()+"]");
						input.attr("type","checkbox");
						input.attr("checked",el.get());
						input.change( fer_cf( function(){
							this.onChange( el, input.is(":checked") );
						}, this ) );
						
						var lbl = $('<label></label>').append( input ).append( " "+((el.description&&el.description.name)?el.description.name:el.getName()) );
						if( el.description&&el.description.description ){
							lbl.mousePopup(new MousePopup( el.description.description ));
						}
						return $("<div class='varBox'></div>").append( lbl );
					},
					getTextbox: function( el ){
						var input = $('<input></input>');
						input.attr("name","fgsc["+el.getName()+"]");
						input.attr("type","text");
						input.attr("value",el.get());
						input.change( fer_cf( function(){
							//CONFIRM HERE
							this.onChange( el, input.val() );
						}, this ) );
						
						var lbl = $('<label></label>').append(((el.description&&el.description.name)?el.description.name:el.getName())+": ").append( input );
						if( el.description&&el.description.description ){
							lbl.mousePopup(new MousePopup( el.description.description ));
						}
						return $("<div class='varBox'></div>").append( lbl );
					...
					addElement: function( element, data ){
						if( element instanceof faarksGrepoSavedConfig.ciVarGroup ){
							if( element.drawable_childs <= 0 )
								return;
							var desc = element.description;
							if( desc && desc.name ){
								data.element.append( "<h"+(data.depth+2)+">"+desc.name+"</h"+(data.depth+2)+">" );
							}
							if( desc && desc.description ){
								data.element.append( "<span class='groupDescription'>"+desc.description+"</span>" );
							}
							var subData = {
								depth: 1,
								element: $("<div class='groupBox'></div>")
							};
							for( var i = 0; i < element.sub_elements.length; i++ ){
								this.addElement( element.sub_elements[i], subData );
							}
							data.element.append( subData.element );
						}else if( true ){
							if( !element.description )
								return;
							
							
							if( element.basic_type === "boolean" ){
								data.element.append( this.getCheckbox( element ) );
							}else
								data.element.append( this.getTextbox( element ) );
						}
					},
					addProject: function( el ){
						if( el.var_group.drawable_childs > 0 ){
							var n = el.sysname.name;
							this.usedInterfacesList.push(n);
							this.usedInterfaces[n] = {
								listItem: $('<li class="'+((this.usedInterfacesList%2)?"odd":"even")+'"></li>'),
								project: el,
								changedData: {}
							};
							
							this.addElement( el.var_group, {
								depth: 0,
								element: this.usedInterfaces[n].listItem
							} );
							
							$("#faarksDataStorage_configListBox").append( this.usedInterfaces[n].listItem );
							
						}
					},
					onupdate: function( ){
						this.cancel();
						//todo: change this to sth like a dialog
					},
					addButton: function( ){
						var list = $("#content div.menu_inner.player_settings ul");
						if( list.length > 0 ){
							var active = faarksUtility.getParam("edit_fds_content");
							list.append( '<li><a href="'+url("player",faarksUtility.getParam("action",null,"index"),{edit_fds_content:1})+'" class="submenu_link"><span class="left"><span class="right"><span class="middle">'+faarksTextSelection.get("fgsc","faarksDataStorage_config_configButtonText")+'</span></span></span></a></li>');
							if( active ){
								$("#menu_inner_active_link").removeAttr("id");
								list.find("li:last").attr("id","menu_inner_active_link");
							}
						}
					},
					init: function(){
						
						if( faarksUtility.getParam("edit_fds_content") ){//($("#faarksDataStorage_mainBox").length > 0)
							faarksGrepoSavedConfig.is_edit_mode = true;
							if( !this.inited ){//draw basic ui
								/*
								return;//DS-OFF
								this.addButton();
								this.drawBasicInterface();
								this.inited = true;
								faarksDataStorage.config.onupdated.push( fer_cf( this.onupdate, this ) );
								*/
							}
							for( var i = 0; i < faarksGrepoSavedConfig.content.length; i++ ){
								var cp = faarksGrepoSavedConfig.content[i];
								if( !this.usedInterfaces[ cp.sysname.name ] ){
									this.addProject( cp );
								}
							}
							if( this.usedInterfacesList.length > 0 ){
								$("#faarksDataStorage_configListEmptyElement").remove();
							}
							
							
						}else if( (Game.controller == "player") && (faarksGrepoSavedConfig.content.length > 0) && !this.inited ){
							this.inited = true;
							this.addButton();
						}
					}
			...
			/* CONSTRUCTOR... USE THAT WITH C....-Objects (or Strings)*/
			init: function( /* projectsysname, projectdescription, varlist */ ){
				if( arguments.length > 0 ){
faarksDataStorage.config.default_options.context = function(){return "grepo_"+faarksGrepoTools.currentWorld+"_pl"+Game.player_id};//TODO: FIX THAT!!!!
					var ret = this.cProject1.apply( this, arguments );
					this.interfaces.grepo.init();
					return ret;
				}
			}
		};
	...
	if( (typeof faarksNewsFetcher === "undefined") ){
		var faarksNewsFetcher = {
			...
			default_news_class: function( news_data ){
				...
				this.persistent = setf(news_data,"persistent",false);
				this.view_from = setf(news_data,"view_from",null);
				this.view_to = setf(news_data,"view_to",null);
				
				if( !this.id ){
					if( this.force_open )
						throw new Error("force open requires msg-id");
			...
			init: function( options ){
				...
				var src = options.source ? options.source : ( options.source_uri ? options.source_uri : nulll );
				if( src ){
	...
	*/
		...
		createDefaultButton: function( text, id ){
			return '<a '+(id?('id="'+id+'" '):'')+' href="#" style="display:inline-block;" class="button"><span class="left"><span class="right"><span class="middle">'+text+'</span></span></span><span style="clear:both;"></span></a>';
...
//								throw new Error( this.durCache[source_town_id][target_town_id].error );
			...
			commandTab: {
				...
				update: function(to_id, from_id){
					if( (this.sourceId == from_id) && (this.targetId == to_id) ){
						this.draw();
					}
				},
				load: function(){
		...
		*/
		moralInPopup: {
			dontUseOnWorlds: [ "de1", "de2", "de3", "de4", "de5", "de6", "de7", "de8", "de9", "en1", "en2", "en3", "en4", "en5", "en6", "en7" ],
			ownMoral: {
				state: null,
				cache: null,
...
/**/		improvedMarket:{
			dataStorage: null,
			getMods: function( buySell ){
				var types = ["wood","iron","stone"];
				var mods = this.dataStorage.get("last_mods");
				if( ($.inArray( buySell.buy, types ) >= 0) && ($.inArray( buySell.sell, types ) ) ){
					if( typeof mods[buySell.buy+"_"+buySell.sell] !== "undefined" ){
						return mods[buySell.buy+"_"+buySell.sell];
					}
				}
				return null;
				/*
				return {
					...
					my_way: 0,
					max_ratio: 0,
					default_sort: "ratio", //["","way","ratio"]
					last_ratio: 1.25 //0
				};
				*/
			},
			setMods: function( buy, sell, mods ){
				var types = ["wood","iron","stone"];
				var mods = this.dataStorage.get("last_mods");
				if( ($.inArray( buy, types ) >= 0) && ($.inArray( sell, types ) ) ){
					mods[buy+"_"+sell] = $.extend( mods[buy+"_"+sell], mods );
					this.dataStorage.set("last_mods",mods);
				}
			},
			onCreateOffer: function(){
				var offer_type = $('#market_create #offer_type_select input').val();
				var offer = $('#market_create #input_offer').val();
				var demand_type = $('#market_create #demand_type_select input').val();
				var demand = $('#market_create #input_demand').val();
				var max_delivery_time = $('#market_create #input_time').val();
				var ratio = Number( demand ) / Number( offer );
				if( !isNaN(ratio) )
					this.setMods( demand_type, offer_type, {
						my_way: max_delivery_time,
						last_ratio: ratio
					});
			},
			onSearch: function(){
				var offer_type = $('#market_search #offer_type_select input').val();
				var demand_type = $('#market_search #demand_type_select input').val();
				var maxway = $('#market_search #time input').val();
				var maxrate = $('#market_search #ratio input').val();
				var order = $('#market_search input#order_by').val();
				this.setMods( demand_type, offer_type, {
					max_way: maxway,
					max_ratio: maxrate,
					default_sort: order
			...
			getBuySellType: function(){
				switch( this.dataStorage.get("default_selection") ){
					case "resources":
						var max=-1,min=-1,maxl,minl=[],res=["wood","stone","iron"];
						for( var i = 0; i < res.length; i++ ){
							if( Layout.resources[ res[i] ] >= max ){//todo: = to "near"
								if( Layout.resources[ res[i] ] > max ){
									maxl = [];
									max = Layout.resources[ res[i] ];
								}
								maxl.push( res[i] );
							}
							if( (min < 0) || (Layout.resources[ res[i] ] <= min ) ){//todo: = to "near"
								if( (min < 0) || (Layout.resources[ res[i] ] < min) ){
									minl = [];
									min = Layout.resources[ res[i] ];
								}
								minl.push( res[i] );
							}
						}
						if( (maxl.length > 1) && (minl.length > 1) )
							return null;
						return {
							buy: (minl.length > 1) ? "" : minl[0],
							sell: (maxl.length > 1) ? "" : maxl[0]
						};
					default:
						return null;
				}
			...
			lastRate_cacheData: {
				...
				buy_changed: false,
				sell_changed: false,
				is_my_changeing: false
			},
			apply_lastRate: function(){
				if( this.lastRate_cacheData.is_my_changeing ){
					//console.log("apply canceld");
					return;
				}
				var buy_type = $('#market_create #demand_type_select input').val();
				var buy = Number($('#market_create #input_demand').val());
				var sell_type = $('#market_create #offer_type_select input').val();
				var sell = Number($('#market_create #input_offer').val());
				if( buy != this.lastRate_cacheData.last_buy )
					this.lastRate_cacheData.buy_changed = true;
				if( sell != this.lastRate_cacheData.last_sell )
					this.lastRate_cacheData.sell_changed = true;
				//console.log("init apply ("+buy+"["+this.lastRate_cacheData.buy_changed+"]=>"+sell+"["+this.lastRate_cacheData.sell_changed+"])");
				if( this.lastRate_cacheData.sell_changed && this.lastRate_cacheData.buy_changed )
					return;
				var mods = this.getMods( {buy: buy_type, sell: sell_type} );
				if (!mods)
					return;
				try{
					this.lastRate_cacheData.is_my_changeing = true;
					var cng = false;
					if( this.lastRate_cacheData.sell_changed ){
						var cngTo = sell * mods.last_ratio;
						if( cngTo != buy ){
							$('#market_create #input_demand').val(cngTo).change();
							cng = true;
						}
					}else if( this.lastRate_cacheData.buy_changed ){
						var cngTo = buy / mods.last_ratio;
						if( cngTo != sell ){
							$('#market_create #input_offer').val(cngTo).change();
							cng = true;
						}
					}
					if( cng ){
						this.lastRate_cacheData.last_buy = Number($('#market_create #input_demand').val());
						this.lastRate_cacheData.last_sell = Number($('#market_create #input_offer').val());
					}
				}finally{
					this.lastRate_cacheData.is_my_changeing = false;
				}
				//console.log("end apply ("+buy+"=>"+sell+")");
			},
			onDataReaded: function(){
				if((Game.controller == "building_market")&&this.dataStorage.get("enabled")){
					if( $("#market_create").length > 0 ){
						//wenn nichts ausgewählt:
						if( $("#market_create #demand_type_select .market_res_select.selected, #market_create #offer_type_select.market_res_select.selected").length <= 0 ){
							var types = this.getBuySellType();
							if( types ){
								if( types.sell )
									$("#market_create #offer_type_select .market_res_select."+types.sell).click();
								if( types.buy )
									$("#market_create #demand_type_select .market_res_select."+types.buy).click();
								var mods = this.getMods( types );
								if( mods && (mods.max_way > 0) ){
									$("#market_create #input_time").val(mods.my_way);
									$("#market_create #input_time").change();
								}
							}
						}
						if( this.dataStorage.get("save_offers") ){
							faarksGrepoTools.setInterrupt( BuildingMarket, "createOwnOffer", this, this.onCreateOffer );
						}
						if( this.dataStorage.get("use_last_rate") ){
							this.lastRate_cacheData.last_buy = Number($('#market_create #input_demand').val());
							this.lastRate_cacheData.last_sell = Number($('#market_create #input_offer').val());
							$("#market_create #slider_demand, #market_create #slider_offer").change( fer_cf( this.apply_lastRate, this ) );
						}
					}else if( $("#market_search").length > 0 ){
						if( $("#market_search #offer_search_form #demand_type_select .market_res_select.selected, #market_search #offer_type_select.market_res_select.selected").length <= 0 ){
							var types = this.getBuySellType();
							if( types ){
								if( types.buy )
									$("#offer_type_select .market_res_select."+types.buy).click();
								if( types.sell )
									$("#demand_type_select .market_res_select."+types.sell).click();
								var mods = this.getMods( types );
								if( mods ){
									switch( mods.default_sort ){
										case "way":
											$("#market_search #distance_icon").click();
											break;
										case "ratio":
											$("#market_search #ratio_icon").click();
											break;
									}
									$("#market_search #input_delivery_time").val(mods.max_way);
									$("#market_search #input_delivery_time").change();
									$("#market_search #input_ratio").val(mods.max_ratio);
									$("#market_search #input_ratio").change();
								}
							}
						}
						if( this.dataStorage.get("auto_apply_search") ){
							$("#market_search div.game_footer a").click();
						}
						if( this.dataStorage.get("save_searches") ){
							$("#market_search div.game_footer a").click(fer_cf( this.onSearch, this ) );
						}
					}
					
					if( this.dataStorage.get("change_res_button_behavior") ){
						$('.market_res_select').unbind('click').bind('click', function() {
							var id = $($(this).parent()).attr('id');
							var res = $(this).attr('class');
							var is = $(this).hasClass('selected');
							res = res.replace(/(\sselected|market_res_select\s)/g, '');
							$('#' + id + ' .market_res_select').removeClass('selected');
							if( is ){
								$('#' + id + ' input').val("");
							}else{
								$('#' + id + ' input').val(res);
						...
						$('#ratio_icon').unbind('click').bind('click', function() {
							if( $(this).hasClass("selected") ){
								$(this).removeClass('selected');
								$('#order_by').val("");
							}else{
								$(this).addClass('selected');
								$('#order_by').val($(this).attr('value'));
							}
							$('#distance_icon').removeClass('selected');
						});
						$('#distance_icon').unbind('click').bind('click', function() {
							if( $(this).hasClass("selected") ){
								$(this).removeClass('selected');
								$('#order_by').val("");
							}else{
							...
							$('#ratio_icon').removeClass('selected');
						});
					}
				}
			},
			init: function(){
				if( faarksGrepoSavedConfig.is_edit_mode || (Game.controller == "building_market") ){
					this.dataStorage = faarksGrepoSavedConfig.init(
						"fg_im", [
							faarksGrepoSavedConfig.cDescription1( "Market-Offers", "Some nice market-improvements..." ),
							faarksGrepoSavedConfig.cVar1( "enabled", false, faarksGrepoSavedConfig.cDescription1("Quoten","Soll diese Funktion aktiv sein?") ),
							faarksGrepoSavedConfig.cVar1( "default_selection", "resources", faarksGrepoSavedConfig.cDescription1("Default selection mode", "ATM nur 'resources' oder 'none'") ), //"
							faarksGrepoSavedConfig.cVar1( "use_last_rate", true, faarksGrepoSavedConfig.cDescription1("Use last Rates") ),
							faarksGrepoSavedConfig.cVar1( "auto_apply_search", false, faarksGrepoSavedConfig.cDescription1("Suchparameter automatisch verwenden") ),
							faarksGrepoSavedConfig.cVar1( "save_offers", true, faarksGrepoSavedConfig.cDescription1("Erstellte Angebote speichern") ),
							faarksGrepoSavedConfig.cVar1( "save_searches", true, faarksGrepoSavedConfig.cDescription1("Erstellte Suchen speichern") ),
							faarksGrepoSavedConfig.cVar1( "change_res_button_behavior", true, faarksGrepoSavedConfig.cDescription1("Ressourcen-Button abändern") ),
							faarksGrepoSavedConfig.cVar1( "last_mods", {}, "object" ),
							faarksGrepoSavedConfig.cVar1( "last_buy", "" ),
							faarksGrepoSavedConfig.cVar1( "last_sell", "" )
						], fer_cf( this.onDataReaded, this ) );
				}
			}
		},
/**/		advancedCommandDetail: {
			dataStorage: null,
			onDataReaded: function(){
			...
			init: function(){
				if( typeof CommandInfo !== "undefined" ){
					this.dataStorage = faarksGrepoSavedConfig.init(
						"fg_acd", [
							faarksGrepoSavedConfig.cDescription1( "Advanced Command Detail", "Let you change the name of commands and displays walktimes" ),
							faarksGrepoSavedConfig.cVar1( "enabled_autoSaveTimes", false, faarksGrepoSavedConfig.cDescription1("Zeiten automatisch speichern","Soll automatisch gespeichert werden, wann ein Angriff das erste mal gesehen wurde?") ),
							faarksGrepoSavedConfig.cVar1( "incs", {}, "object" )
						], fer_cf( this.onDataReaded, this ) );
				}
			}
		},
	...
	.faarksGrepoTools_featureListMenuButton.fgt_flmb_open:hover{ background-position: -144px -19px; }\
			...
			add: function( options ){
				...
				data.create.apply( this, [this.getPopupElement().find(".fgt_flmb_content"), data] );
			}
		},
		silverList: {
			data: {
			...
			view_data: function(){
				...
				if( this.data.town_count > 0 ){
					...
					alert( fgt.getString("silverList_contentMsg_layout",{content:e}) );
				}
			},
			run: function(){
		...
		/*newsGui: {
			...
			get_gui: function(){
				...
				if( box.length <= 0 ){
					$('head').append("<style type=\"text/css\">\
						#faark_newsGui_grepo{ z-index: 10; position: absolute; top: 0pt; left: 0pt; display: block; }\
					...
					box.find(".fns_headline .fns_itemcontent").text( fgt.getString("newsGui_headline_text") ).mousePopup( new MousePopup( fgt.getString("newsGui_headline_popup") ) );
					$("#box #menu").before( box );
				}
				return box;
			},
			...
			open_details: function( news_element ){
				if( this.news_list_windows[ news_element.id ] )
					return;
				var win = fgt.createWindow({
					onclose: fer_cf(function(){
						this.news_list_windows[ news_element.id ] = null;
					},this)
				});
				this.news_list_windows[ news_element.id ] = win.boxElement;
				win.contentElement.append("<h2>"+news_element.title+"</h2><div>"+news_element.message+"</div>");
			},
			...
			on_new_news: function( news_element ){
				...
				},this)).find(".fns_itemcontent").text( news_element.title ? news_element.title : "No Title" );
